    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        for multiplier in self.buy_m2.range:
            for period in self.buy_p2.range:
                calc_dict['supertrend_2_buy_' + str(multiplier) + "_" + str(period)] = self.supertrend(dataframe, multiplier, period)[
                    'STX']
                # dataframe[f'supertrend_2_buy_{multiplier}_{period}'] = self.supertrend(dataframe, multiplier, period)[
                #     'STX']
        for multiplier in self.buy_m3.range:
            for period in self.buy_p3.range:
                calc_dict['supertrend_3_buy_' + str(multiplier) + "_" + str(period)] = self.supertrend(dataframe, multiplier, period)[
                    'STX']
        ...
        for multiplier in self.sell_m2.range:
            for period in self.sell_p2.range:
                calc_dict['supertrend_2_sell_' + str(multiplier) + "_" + str(period)] = self.supertrend(dataframe, multiplier, period)[
                    'STX']
                # dataframe[f'supertrend_2_sell_{multiplier}_{period}'] = self.supertrend(dataframe, multiplier, period)[
                #     'STX']
        for multiplier in self.sell_m3.range:
            for period in self.sell_p3.range:
                calc_dict['supertrend_3_sell_' + str(multiplier) + "_" + str(period)] = self.supertrend(dataframe, multiplier, period)[
                    'STX']
                # dataframe[f'supertrend_3_sell_{multiplier}_{period}'] = self.supertrend(dataframe, multiplier, period)[
                #     'STX']
        supertrend_data = DataFrame.from_dict(calc_dict)
        dataframe = concat([dataframe, supertrend_data],axis=1)
        print(metadata)
